...
let clientNameEdit = document.getElementById("clientNameEdit");
let clientPhoneEdit = document.getElementById("clientPhoneEdit");
let clientMailEdit = document.getElementById("clientEmailEdit");
let exitEditClientBtn = document.getElementById("exitEditClientBtn");
...
function editClient(e) {
    ...
    originalName.value = e.target.dataset.name;
    clientNameEdit.placeholder = e.target.dataset.name;
    clientPhoneEdit.placeholder = e.target.dataset.phone;
    clientMailEdit.placeholder = e.target.dataset.email;
...
function deleteClient(e) {
    fetch("ajax-delete-client.php",
        ...
        }).then(res => res.json()).then(createContent())
}

editClientSection.classList.add("invisible");
exitEditClientBtn.addEventListener('click', exitEditClient);
...
for (let i = 0; i < events[i]; i++) {
    events[i].addEventListener('click', eventPopup)
}